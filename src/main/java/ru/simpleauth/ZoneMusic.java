 ...
 * играющего звука без его перезапуска с начала — поэтому плавного нарастания
 * громкости внутри одного трека здесь нет: это сделало бы звук хуже
 * (заметный рестарт), а не плавнее. Мгновенное переключение (смена трека,
 * смена громкости) — честная и единственная безопасная опция.
 */
public class ZoneMusic {
    ...
    /**
     * Применяет изменённую громкость немедленно ко всем, кто сейчас слушает
     * музыку в зоне — останавливает и тут же запускает тот же трек заново
     * на новой громкости. Без этого громкость менялась только в конфиге и
     * реально применялась лишь при следующем натуральном перезапуске трека
     * (то есть могла не ощущаться минутами) — это и была причина жалобы
     * "громкость не работает".
     */
    public void applyVolumeNow(MinecraftServer server, float newVolume) {
        Config cfg = config();
