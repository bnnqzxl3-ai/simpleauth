...
public class ShopScreenHandler extends GenericContainerScreenHandler {
    ...
    private static final int QTY_SLOT = 37;
    private static final int NEXT_SLOT = 38;
    ...
    public ShopScreenHandler(int syncId, PlayerInventory playerInventory,
                              List<Item> items, int page, int qtyIndex) {
        super(ScreenHandlerType.GENERIC_9X6, syncId, playerInventory,
                buildInventory(items, page, qtyIndex), ROWS);
        this.items = items;
        this.page = page;
    ...
    private static SimpleInventory buildInventory(List<Item> items, int page, int qtyIndex) {
        ...
        search.set(DataComponentTypes.CUSTOM_NAME,
                Text.literal("🔍 Поиск").formatted(Formatting.AQUA, Formatting.BOLD).styled(s -> s.withItalic(false)));
        ...
        pageInfo.set(DataComponentTypes.CUSTOM_NAME,
                Text.literal("Страница " + (page + 1) + "/" + totalPages(items))
    ...
    private void reopen(ServerPlayerEntity player, int newPage, int newQtyIndex) {
        ...
        player.openHandledScreen(new SimpleNamedScreenHandlerFactory(
                (syncId2, inv2, p2) -> new ShopScreenHandler(syncId2, inv2, items, newPage, newQtyIndex),
                Text.literal("Склад")));
    }
