 ...
 * с поиском по названию и переключаемым количеством за клик.
 *
 * Открывается ПОСЛЕ выбора категории (см. ShopCategoryScreen) — category
 * тут только для кнопки "назад" и подписи, список items уже отфильтрован
 * заранее. Поиск (SEARCH_SLOT) ищет по всем предметам разом, не только
 * внутри текущей категории — так удобнее, если не помнишь, в какой она.
 *
 * Все клики по каталогу и кнопкам перехвачены — как и в остальных меню
 * проекта (см. SelectionScreen), реальный дрэг предметов тут не нужен.
 */
public class ShopScreenHandler extends GenericContainerScreenHandler {
    ...
    public ShopScreenHandler(int syncId, PlayerInventory playerInventory,
                              List<Item> items, int page, int qtyIndex, ShopCategory category) {
        super(ScreenHandlerType.GENERIC_9X6, syncId, playerInventory,
                buildInventory(items, page, qtyIndex, category), ROWS);
        this.items = items;
        this.page = page;
    ...
    /** Полный список предметов игры, отсортированный по идентификатору. Строится один раз. */
    ...
    private static int totalPages(List<Item> items) {
        return Math.max(1, (items.size() + PAGE_SIZE - 1) / PAGE_SIZE);
    }

    private static SimpleInventory buildInventory(List<Item> items, int page, int qtyIndex, ShopCategory category) {
        ...
        for (int i = 0; i < PAGE_SIZE; i++) {
            ...
            inv.setStack(i, new ItemStack(items.get(idx)));
        }
        ...
        search.set(DataComponentTypes.CUSTOM_NAME,
                Text.literal("🔍 Поиск (по всем категориям)").formatted(Formatting.AQUA, Formatting.BOLD)
                        .styled(s -> s.withItalic(false)));
        ...
        pageInfo.set(DataComponentTypes.CUSTOM_NAME,
                Text.literal(catLabel + " — стр. " + (page + 1) + "/" + totalPages(items))
    ...
    public void onSlotClick(int slotIndex, int button, SlotActionType actionType, PlayerEntity player) {
        ...
        if (slotIndex == BACK_SLOT) {
            ...
            serverPlayer.openHandledScreen(new SimpleNamedScreenHandlerFactory(
                    (syncId2, inv2, p2) -> new ShopCategoryScreen(syncId2, inv2),
                    Text.literal("Категории")));
            return;
        }
        if (slotIndex == PREV_SLOT) {
            if (page > 0) reopen(serverPlayer, page - 1, qtyIndex);
            return;
        }
        if (slotIndex == NEXT_SLOT) {
            if (page + 1 < totalPages(items)) reopen(serverPlayer, page + 1, qtyIndex);
            return;
        }
        if (slotIndex == QTY_SLOT) {
            reopen(serverPlayer, page, (qtyIndex + 1) % QUANTITIES.length);
            return;
        }
        if (slotIndex == SEARCH_SLOT) {
            serverPlayer.closeHandledScreen();
            serverPlayer.openHandledScreen(new SimpleNamedScreenHandlerFactory(
                    (syncId2, inv2, p2) -> new ItemSearchScreen(syncId2, inv2, allItems(), qtyIndex),
        ...
        if (slotIndex >= 0 && slotIndex < PAGE_SIZE) {
            ...
            if (idx < items.size()) {
                giveItems(serverPlayer, items.get(idx), QUANTITIES[qtyIndex]);
            }
        }
    ...
    private void reopen(ServerPlayerEntity player, int newPage, int newQtyIndex) {
        player.closeHandledScreen();
        player.openHandledScreen(new SimpleNamedScreenHandlerFactory(
                (syncId2, inv2, p2) -> new ShopScreenHandler(syncId2, inv2, items, newPage, newQtyIndex, category),
                Text.literal(category != null ? category.label : "Склад")));
    }

    static void giveItems(ServerPlayerEntity player, Item item, int amount) {
        int remaining = amount;
        while (remaining > 0) {
            int take = Math.min(remaining, item.getMaxCount());
