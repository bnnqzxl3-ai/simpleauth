...
public class AuthCommands {
    ...
    private static void sendCosList(CommandContext<ServerCommandSource> ctx) {
        ...
        section(ctx, "Наборы", "сразу вид и частицы", CosmeticsManager.comboList(), Formatting.GOLD);
        section(ctx, "Виды", "форма", CosmeticsManager.typeList(), Formatting.AQUA);
        section(ctx, "Частицы", "из чего нарисовано", CosmeticsManager.particleList(), Formatting.GREEN);
    ...
    private static int applyCosmetic(AuthManager manager, CommandContext<ServerCommandSource> ctx,
                                     ServerPlayerEntity player, String value) {
        String name = player.getGameProfile().getName();

        if (CosmeticsManager.isCombo(value)) {
            String[] pair = CosmeticsManager.combo(value);
        ...
        if (CosmeticsManager.isType(value)) {
            manager.cosmetics().set(name, value, null);
        } else if (CosmeticsManager.isParticle(value)) {
            ...
            var node = dispatcher.register(CommandManager.literal("cos")
                    ...
                    .then(CommandManager.literal("combo")
                            .then(CommandManager.argument("combo", StringArgumentType.word())
                                    .executes(ctx -> {
                                        ...
                                        if (!CosmeticsManager.isCombo(combo)) {
                                            feedback(ctx, String.format(manager.config().messages.cosBadCombo,
                                                    CosmeticsManager.comboList()), Formatting.RED);
                                            return 0;
                                        }
                                        String[] pair = CosmeticsManager.combo(combo);
                    ...
                    .then(CommandManager.argument("value_or_type", StringArgumentType.word())
                            .suggests((context, builder) ->
                                    CommandSource.suggestMatching(CosmeticsManager.allNames(), builder))
                            .executes(ctx -> {
                                ServerPlayerEntity player = ctx.getSource().getPlayer();
                                if (player == null) return 0;
     ...
     */
    private static void registerPhysics(AuthManager manager) {
     ...
     */
    private static void registerDeathTest(AuthManager manager) {
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) ->
    ...
    private static int runDeathTest(AuthManager manager, CommandContext<ServerCommandSource> ctx) {
        ServerPlayerEntity player;
        try {
            player = ctx.getSource().getPlayer();
        } catch (Exception e) {
        ...
        // Профиль фейка: имя = ник игрока (для скины), но со случайным UUID,
        // иначе сервер сочтёт его тем же аккаунтом и не пустит второго.
        String nick = player.getGameProfile().getName();
        GameProfile profile = new GameProfile(UUID.randomUUID(), nick);

        ServerPlayerEntity fake;
        try {
            // 1.21.1: конструктор принимает опции клиента. Если сигнатура
            // отличается в твоих маппингах — правь эту строку.
            fake = new ServerPlayerEntity(server, world, profile,
                    SyncedClientOptions.createDefault());
        } catch (Throwable t) {
            feedback(ctx, "Не удалось создать фейк-игрока: " + t.getMessage(), Formatting.RED);
            return 0;
        }

        fake.refreshPositionAndAngles(sx, sy, sz, yaw, 0.0F);
        fake.setHealth(20.0F);

        // Подключаем фейка «локальным» соединением, без реального сокета.
        try {
            ClientConnection connection = new ClientConnection(NetworkSide.CLIENTBOUND);
            server.getPlayerManager().onPlayerConnect(
                    connection, fake,
                    new ConnectedClientData(profile, 0,
                            fake.getClientOptions(), false));
        } catch (Throwable t) {
            feedback(ctx, "Не удалось подключить фейк-игрока: " + t.getMessage(), Formatting.RED);
            return 0;
        }

        fake.teleport(world, sx, sy, sz, java.util.Set.of(), yaw, 0.0F, false);
        feedback(ctx, "Фейк-игрок создан — эффект через секунду.", Formatting.AQUA);

        final double fx = sx, fy = sy, fz = sz;
        final float fyaw = yaw;
        final ServerPlayerEntity target = fake;
        manager.schedule(20, srv -> {
            manager.deathShow().start(
                    UUID.randomUUID(), world, fx, fy, fz, fyaw, 0.0F, false);
            try {
                target.networkHandler.disconnect(Text.literal("test done"));
