...
public class AuthCommands {
    ...
    private static void registerDeathTest(AuthManager manager) {
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) ->
                dispatcher.register(CommandManager.literal("deathshowtest")
    ...
    private static int runDeathTest(AuthManager manager, CommandContext<ServerCommandSource> ctx) {
        ...
        // ставим NPC на пару блоков перед взглядом игрока
        Vec3d dir = player.getRotationVec(1.0F);
        double sx = player.getX() + dir.x * 2.0;
        double sy = player.getY();
        double sz = player.getZ() + dir.z * 2.0;

        ZombieEntity npc = EntityType.ZOMBIE.create(world, SpawnReason.COMMAND);
        if (npc == null) {
            feedback(ctx, "Не удалось создать NPC.", Formatting.RED);
            return 0;
        }
        npc.refreshPositionAndAngles(sx, sy, sz, player.getYaw() + 180.0F, 0.0F);
        npc.setCustomName(Text.literal("§bПодопытный"));
        ...
        feedback(ctx, "NPC создан — эффект через секунду.", Formatting.AQUA);

        // через 20 тиков запускаем эффект на месте NPC и убираем его
        final double fx = sx, fy = sy, fz = sz;
        final float fyaw = npc.getYaw();
        manager.schedule(20, server -> {
            manager.deathShow().start(
                    java.util.UUID.randomUUID(), world, fx, fy, fz, fyaw, 0.0F, false);
            if (npc.isAlive()) {
                npc.discard();
            }
        });
