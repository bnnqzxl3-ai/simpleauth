...
public class AuthManager {
    ...
    private final PrefixManager prefixes = new PrefixManager(this);
    private final ShowManager shows = new ShowManager(this);
    private final CosmeticsManager cosmetics = new CosmeticsManager(this);
    private final HatManager hats = new HatManager(this);
    private final RegionCleaner cleaner = new RegionCleaner();
    private volatile boolean physicsDisabled = false;
    private final DeathShow deathShow = new DeathShow(this);
    private final ZoneMusic zoneMusic = new ZoneMusic(this);
    private final ItemCleaner itemCleaner = new ItemCleaner(this);
    private final ServerOptimizer serverOptimizer = new ServerOptimizer(this);
    private final ActionLogger actionLogger = new ActionLogger(this);
    private final AntiCheatMonitor anticheat = new AntiCheatMonitor(this, actionLogger);
    private final NicknameShimmer nicknameShimmer = new NicknameShimmer(this);
    private final AutoTrackSwitcher autoTrackSwitcher = new AutoTrackSwitcher(this);
        ...
        while (it.hasNext()) {
            ...
            if (--s.ticksLeft <= 0) {
                ...
                try {
                    ...
                    SimpleAuth.LOGGER.warn("[SimpleAuth] Ошибка в отложенной задаче", e);
                }
            }
        }
    }
    ...
    public HatManager hats() {
        return hats;
    }

    public RegionCleaner cleaner() {
        return cleaner;
    }

    public DeathShow deathShow() {
        return deathShow;
    }
    ...
    public ItemCleaner itemCleaner() {
        return itemCleaner;
    }

    public ServerOptimizer serverOptimizer() {
    ...
    public AntiCheatMonitor anticheat() {
        return anticheat;
    }

    public NicknameShimmer nicknameShimmer() {
        return nicknameShimmer;
    }

    public AutoTrackSwitcher autoTrackSwitcher() {
        return autoTrackSwitcher;
    }
    ...
    private void checkAddress(ServerPlayerEntity player, String name) {
        ...
        SimpleAuth.LOGGER.warn("[SimpleAuth] Вход {} с нового адреса {}", name, masked);
