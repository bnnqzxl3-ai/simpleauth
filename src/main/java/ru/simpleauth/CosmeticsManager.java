...
public class CosmeticsManager {
    ...
    private void spawn(ServerWorld world, ParticleEffect effect, double x, double y, double z) {
        if (budget <= 0) return;
        budget--;
        world.spawnParticles(effect, x, y, z, 1, 0.0, 0.0, 0.0, 0.0);
    }

    /** Частица с заданной скоростью — для фонтана и дождя. */
    private void spawnMoving(ServerWorld world, ParticleEffect effect, double x, double y, double z,
                             double dx, double dy, double dz, double speed) {
        if (budget <= 0) return;
        budget--;
        world.spawnParticles(effect, x, y, z, 0, dx, dy, dz, speed);
    }
